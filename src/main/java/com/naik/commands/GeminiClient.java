...
public class GeminiClient {
    ...
    private static String buildRequestBody(String prompt) {
        ...
        return "{"
                ...
                + "        \"description\": \"Choose an action if user requested it. Otherwise select NONE.\""
                + "      },"
                + "      \"actionTarget\": {"
                + "        \"type\": \"STRING\","
                + "        \"description\": \"The argument for the selected action (e.g. app name, website domain, search query).\""
                + "      }"
                + "    },"
                ...
                + "},"
                + "\"systemInstruction\": {"
                + "  \"parts\": [{\"text\": \"You are NAIK, a highly active, reactive, intelligent voice assistant for the user Keshav. You run on his Windows desktop. Provide direct responses to help him. Choose the appropriate action if he asks you to open a program, launch a website, search Google, search YouTube, or search Wikipedia. If it is just a friendly conversation or general question, choose NONE for action. Always output valid JSON conforming to the schema.\"}]"
                + "}"
                + "}";
    }
